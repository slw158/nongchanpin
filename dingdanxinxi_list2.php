...
<form id="form1" name="form1" method="post" action="">
    ...
    账号：<input name="zhanghao" type="text" id="zhanghao" style='border:solid 1px #000000; color:#666666;width:80px'/>
    排序
...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="#00FFFF"
    ...
    <tr>
        ...
        <td bgcolor='#CCFFFF' width='80' align='center'>是否发货</td>
        <td width="120" align="center" bgcolor="#CCFFFF">添加时间</td>
        <td width="70" align="center" bgcolor="#CCFFFF">操作</td>
    </tr>
    <?php
    $sql = "select * from dingdanxinxi where zhanghao='" . $_SESSION['username'] . "'";

    if ($_POST["dingdanhao"] != "") {
        $nreqdingdanhao = $_POST["dingdanhao"];
        $sql = $sql . " and dingdanhao like '%$nreqdingdanhao%'";
    }
    if ($_POST["dingdanjine"] != "") {
        $nreqdingdanjine = $_POST["dingdanjine"];
        $sql = $sql . " and dingdanjine like '%$nreqdingdanjine%'";
    }
    if ($_POST["zhanghao"] != "") {
        $nreqzhanghao = $_POST["zhanghao"];
        $sql = $sql . " and zhanghao like '%$nreqzhanghao%'";
    }
    if ($_POST["paixu"] != "") {
    ...
    if ($rowscount == 0) {
        ...
        for ($i = $pagecurrent * $pagelarge - $pagelarge; $i < $ddddd; $i++) {
            ...
            <tr>
                ...
                <td align='center'><?php echo mysql_result($query, $i, iszf); ?>  <?php if (mysql_result($query, $i, iszf) == "否") { ?>
                        <a href="zhifu/index.php?id=<?php echo mysql_result($query, $i, id); ?>&biao=dingdanxinxi">
                            支付</a> <?php } ?> </td>
                <td align='center'>
                    <a>
<!--                            href="sh.php?id=--><?php //echo mysql_result($query, $i, "id") ?><!--&yuan=--><?php //echo mysql_result($query, $i, "issh") ?><!--&tablename=dingdanxinxi"-->

                    <?php echo mysql_result($query, $i, "issh") ?></a>
                </td>
                <td width="120" align="center"><?php echo mysql_result($query, $i, "addtime"); ?></td>
                <td width="70" align="center">
                    <?php if (mysql_result($query, $i, issh) == "否") { ?><a
                        href="del.php?id=<?php echo mysql_result($query, $i, "id"); ?>&tablename=dingdanxinxi"
                        onclick="return confirm('真的要删除？')">删除</a> <a
                            href="dingdanxinxi_updt.php?id=<?php echo mysql_result($query, $i, "id"); ?>">
                            修改</a><?php } ?>
...
<p align="center"><a href="dingdanxinxi_list2.php?pagecurrent=1">首页</a>, <a
            href="dingdanxinxi_list2.php?pagecurrent=<?php echo $pagecurrent - 1; ?>">前一页</a> ,<a
            href="dingdanxinxi_list2.php?pagecurrent=<?php echo $pagecurrent + 1; ?>">后一页</a>, <a
            href="dingdanxinxi_list2.php?pagecurrent=<?php echo $pagecount; ?>">末页</a>,
