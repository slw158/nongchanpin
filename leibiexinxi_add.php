...
if ($addnew=="1" )
{
	...
	ischongfu("select id from leibiexinxi where  leibie='$leibie'");
	$sql="insert into leibiexinxi(leibie) values('$leibie') ";
	mysql_query($sql);
...
<body>
<p>添加类别信息： 当前日期： <?php echo $ndate; ?></p>
<script language="javascript">
	function check()
{
	...
	function gow()
	{
		location.href='leibiexinxi_add.php?jihuabifffanhao='+document.form1.jihuabifffanhao.value+"&id=<?php echo $_GET["id"]?>";
	}
...
<?php
	function ischongfu($sql)
	{
		...
		if($rowscount>0)
		{
			echo "<script>javascript:alert('对不起，您输入的类别已经存在，请重试!');history.back();</script>";
			exit;
		}
	}
?>
